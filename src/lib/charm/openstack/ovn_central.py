...
CRONJOB_CMD = "{schedule} {command} 2>&1 | logger -p local0.notice\n"
...
class BaseOVNCentralCharm(charms_openstack.charm.OpenStackCharm):
    ...
    def add_nrpe_certs_check(self, charm_nrpe):
        ch_core.host.rsync(os.path.join(os.getenv('CHARM_DIR'), 'files',
                                        'nagios', 'nrpe_check_ovn_certs.py'),
                           os.path.join(NAGIOS_PLUGINS,
                                        'nrpe_check_ovn_certs.py'))
        check_cmd = 'nrpe_check_ovn_certs.py'
        ...
        rsync(os.path.join(os.path.join(os.getenv('CHARM_DIR'), 'files',
                                        'scripts', 'check_ovn_certs.py'),
              os.path.join(SCRIPTS_DIR, 'check_ovn_certs.py')))
        cronjob = CRONJOB_CMD.format(
            schedule='*/5 * * * *',
            command=os.path.join(SCRIPTS_DIR, 'check_ovn_certs.py'))
