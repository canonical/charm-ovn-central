...
# NOTE(fnordahl): We need to carry fixes for the ``cluster_status`` and
# ``is_northd_active`` helpers awaiting a c-h release that contains
# commit b4aa4e3398e7406dbf0f76a23f91afa6a72aed1a
    ...
    for line in ch_ovn.ovn_appctl(
        ...
        if k and line.startswith(' '):
            ...
            if v:
                ...
                status[k] = []
    return ch_ovn.OVNClusterStatus(
        status['name'],
...
def is_northd_active():
    ...
    """
    try:
        for line in ch_ovn.ovn_appctl('ovn-northd', ('status',)).splitlines():
...
class BaseOVNCentralCharm(charms_openstack.charm.OpenStackCharm):
    ...
    def cluster_status(self, db):
        ...
        try:
            ...
            return cluster_status(db, rundir=self.ovn_rundir(),
                                  use_ovs_appctl=(
                                      self.release == 'train'))
