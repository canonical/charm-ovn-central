...
class TestRunOVNChecks(test_utils.PatchHelper):
    ...
    def test_run_checks_sb_not_leader(self, mock_leader, mock_write):
        mock_leader.return_value = False
        check.run_checks(self.SOUTHBOUND)
        mock_write.assert_called_once_with(
            "OK: no-op (unit is not the DB leader)",
            "/var/lib/nagios/ovn_sb_db_connections.out"
        )

    @mock.patch('run_ovn_db_connections_check.write_output_file')
    @mock.patch('run_ovn_db_connections_check.is_leader')
    def test_run_checks_nb_not_leader(self, mock_leader, mock_write):
        mock_leader.return_value = False
        check.run_checks(self.NORTHBOUND)
        mock_write.assert_called_once_with(
            "OK: no-op (unit is not the DB leader)",
            "/var/lib/nagios/ovn_nb_db_connections.out"
    ...
    @mock.patch('run_ovn_db_connections_check.is_leader')
    def test_run_checks_sb_leader(self, mock_leader, mock_aggregate,
                                  mock_parse, mock_check, mock_check_output,
                                  mock_write):
    ...
    @mock.patch('run_ovn_db_connections_check.aggregate_alerts')
    @mock.patch('run_ovn_db_connections_check.is_leader')
    def test_run_checks_nb_leader(self, mock_leader, mock_aggregate,
                                  mock_parse, mock_check, mock_check_output,
                                  mock_write):
        mock_leader.return_value = True
        mock_aggregate.return_value = "OK: fake status"
    ...
    def test_check_role_target_sb_unexpected_role(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "fakerole",
            "target": "pssl:6642",
        }
        alert = check.check_role_target(connection, self.SOUTHBOUND)
        self.assertEquals(alert.status, check.NAGIOS_STATUS_CRITICAL)

    def test_check_role_target_sb_unexpected_target(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "",
            "target": "pssl:26642",
    ...
    def test_check_role_target_nb_unexpected_target(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "",
            "target": "pssl:26642",
        }
        alert = check.check_role_target(connection, self.NORTHBOUND)
        self.assertEquals(alert.status, check.NAGIOS_STATUS_CRITICAL)

    def test_check_role_target_sb_rbac_disabled(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "",
            "target": "pssl:6642",
    ...
    def test_check_role_target_sb_ovn_controller_rbac(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "ovn-controller",
            "target": "pssl:16642",
    ...
    def test_check_role_target_sb_ok_ovn_controller(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "ovn-controller",
            "target": "pssl:6642",
    ...
    def test_check_role_target_sb_rbac_ok(self):
        connection = {
            "_uuid": ["uuid", "fake-uuid"],
            "role": "",
            "target": "pssl:16642",
    ...
    def test_check_connections_sb_too_many(self):
        connections = [
            {
        ...
        alerts = check.check_connections(connections, self.SOUTHBOUND)
        self.assertIn(
            check.Alert(
    ...
    def test_check_connections_sb_too_many_controllers(self):
        connections = [
            {
                "_uuid": ["uuid", "fake-uuid-0"],
                "role": "",
    ...
    def test_aggregate_alerts_sb(self):
        ...
        filtered1 = check.aggregate_alerts(alerts1, self.SOUTHBOUND)
        self.assertEquals(
            filtered1,
        ...
        filtered2 = check.aggregate_alerts(alerts2, self.SOUTHBOUND)
        self.assertEquals(
        ...
        filtered3 = check.aggregate_alerts(alerts3, self.SOUTHBOUND)
        self.assertEquals(filtered3, "OK: OVN SB DB connections are normal")
    ...
    def test_aggregate_alerts_nb(self):
        ...
        filtered1 = check.aggregate_alerts(alerts1, self.NORTHBOUND)
        self.assertEquals(
            filtered1,
            "CRITICAL: critical[1]: ['fakecrit']; "
            "warnings[2]: ['fakewarn1', 'fakewarn2']",
        ...
        filtered2 = check.aggregate_alerts(alerts2, self.NORTHBOUND)
        self.assertEquals(
            filtered2, "WARNING: warnings[2]: ['fakewarn1', 'fakewarn2']"
        )
        ...
        filtered3 = check.aggregate_alerts(alerts3, self.NORTHBOUND)
        self.assertEquals(filtered3, "OK: OVN NB DB connections are normal")
